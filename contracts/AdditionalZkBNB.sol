    ...
    */
    ...
    )
    {
        ...
        for (uint16 i = 0; i < _newBlockData.publicDataOffsets.length; ++i) {
            ...
            console.log("INFO! txType=%s,uncommittedPriorityRequestsOffset=%s,priorityOperationsProcessed=%s",
                uint(txType), uncommittedPriorityRequestsOffset, priorityOperationsProcessed);
    ...
    function checkPriorityOperation(TxTypes.FullExitNft memory _fullExitNft, uint64 _priorityRequestId) internal view {
        ...
        if (priorReqType != TxTypes.TxType.FullExitNft) {
            console.log("ERROR! priorReqId=%s,priorReqType=%s,required=FullExitNft", _priorityRequestId, uint(priorReqType));
        }
